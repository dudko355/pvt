package by.pvt.dudko.company.web.impl.constant;

public class ConstantsPages {
	public static final String PAGES_LOGIN = "login";
	public static final String PAGES_REGISTR = "registration";
	public static final String PAGES_ADMIN = "admin";
	public static final String LOCALE = "index";
	public static final String PAGES_CLIENT = "client_page";
	public static final String ERROR = "error";
	public static final String PAGES_BASE = "start_page";
	public static final String PAGES_ORDER = "page_order";
	public static final String ORDERS = "orders";
	public static final String ADMIN_INF = "admin_inf";
	public static final String CHANGE_TRIP = "change_trip";
	public static final String CHANGE_CAR_COND = "change_car";

}
